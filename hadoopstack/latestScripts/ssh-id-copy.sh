...
jobtracker=`cat $properties_file | grep -i jobtracker | tr -s ' ' |cut -f 2 -d ' '| cut -d ":" -f 1`
...
for master in $jobtracker
do
	scp -i $identity_file -o StrictHostKeyChecking=no ~/.ssh/id_rsa.pub $user@$master:
	ssh -i $identity_file -o StrictHostkeyChecking=no $user@$master "cat id_rsa.pub >> ~/.ssh/authorized_keys"
done
for slave in $slaves
do
